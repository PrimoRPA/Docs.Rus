...
> Начиная с версии 1.25.1 в разделе RPA-проекты добавлено отображение учетной записи, от имени которой опубликован или загружен проект. Учетная запись отображается под датой в столбце "Дата создания".  
> Кроме того, добавлена возможность удалять приоритет в формах создания и редактирования проекта. Новый проект по умолчанию создаётся со средним приоритетом.  
> Также была добавлена кнопка "Скачать в CSV" для экспорта данных.
...
* **Не повторять в очереди проектов** означает, что, если проект уже находится в очереди проектов, повторно в очередь проект он добавлен не будет. Рекомендуется использовать этот параметр, чтобы не спровоцировать неоправданный рост очереди проектов. Начиная с версии 1.25.1 добавлена возможность автоматического применения настройки **Не повторять в очереди проектов** для всех новых проектов. Теперь настройка по умолчанию активирована на форме добавления RPA-проекта, исключая необходимость её ручного включения для каждого проекта.
