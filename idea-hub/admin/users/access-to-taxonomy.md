...
Рассмотрим пример настройки доступа к терминам словаря **Департаменты/Управления**. В данном примере мы хотим, чтобы любой авторизованный пользователь имел право на просмотр только того департамента, который был установлен в его учётной записи в поле **Подразделение**.
...
1. В блоке операции **Читать** настроим следующие параметры:
   * **Тип правила** — выберем значение `Термин таксономии выбранный для пользователя`.
   * **Роли с доступом** — оставим значение `Authenticated user`.
   * **Поле пользователя** — выберем значение `Подразделение`, которое ссылается на словарь **Департаменты/Управления**.
   * **Включить термины** — выберем значение `Только выбранный термин`.
1. Нажмём **Сохранить правила** внизу страницы.

В результате, когда пользователь зайдет в Idea Hub, то на странице **Департаменты** он увидит только департамент из своей учётной записи. Например, если в учётной записи, в поле **Подразделение**, стоит значение **Команда RPA**, значит, только этот департамент он увидит в таблице департаментов. Все другие департаменты ему будут недоступны.
