...
1. Проверьте работоспособность Idea Hub на странице **Отчёты > Отчёт о состоянии**. Здесь вы можете проверить версию Drupal, версию базы данных, веб-сервера, версию PHP, лимит памяти PHP, а также просмотреть, когда в последний раз запускался скрипт cron. Этот скрипт должен выполняться регулярно, чтобы Idea Hub работал должным образом. Отчёт не должен содержать ошибок и предупреждений — в ином случае обратитесь с разработчикам Idea Hub, чтобы исправить состояние веб-приложения. 
1. Определите основные настройки сайта, включая страну и часовой пояс по умолчанию.
