...
В скрипте `get_data.sh` для обозначения контуров используется слово environments. Название контуров, которые вы создадите, следует указать при настройке файла `.env` для корректной работы скрипта `get_data.sh`.
