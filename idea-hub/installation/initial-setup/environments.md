...
Существует возможность добавить дополнительный источник данных для контура. Например, вы можете собирать только часть данных из Оркестратора, а для остальных данных использовать другой источник.

С версии Idea Hub 25.3 появилась возможность использовать в качестве источника данных БД ltoolsanalytic — аналитическую систему Оркестратора, которая предоставляет более точную статистику запусков роботов и проектов. Если у вас установлена эта версия, то рекомендуем подключить ее в качестве дополнительного импортера для сбора указанной статистики. 
...
1. Предварительно убедитесь, что учетные данные целевой базы данных указаны в файле `settings.local.php`.
   ...
   ```
1. В веб-интерфейсе Idea Hub, после нажатия кнопки **Добавить импортер** выберите в качестве **Типа исходных данных**:
   * **Primo Analytic** — аналитическая система Оркестратора для сбора статистики по запускам проектов и роботов.
   * **Metadata** — внешняя целевая БД. 
1. Укажите время начала импорта исходных данных и дополнительные параметры.

   Например, для **Primo Analytic** установите галочки напротив данных, которые вы хотите импортировать:
   * Статистика проектов — если галочка установлена, то для импортера **Primo RPA Global** должна отсутствовать галочка напротив данных **ProjectStats**.
   * Статистика роботов — если галочка установлена, то для импортера **Primo RPA Global** должна отсутствовать галочка напротив данных **RobotStats**.
