...
Все параметры скрипта `get_data.sh` указаны в файле `.env.example`. С описанием параметров можно ознакомиться ниже либо в самом файле.
...
Следующий шаг — перейти в веб-интерфейс Idea Hub, чтобы [создать контуры организации](https://docs.primo-rpa.ru/primo-rpa/primo-rpa-idea-hub/installation/initial-setup/environments) и указать для них каталоги, из которых Idea Hub будет забирать полученные данные из Оркестратора.
