...
Как устроен `get_data.sh` и что где настраивается: С помощью скрипта подключаемся к БД Оркестратора, создаёт папку для данных этого окружения, получает данные, складывает их в формате CSV в папку контура. Рядом с CSV-файлами сохраняется log-файл с процессом получения данных.

1. Скрипт `get_data.sh` запускается по cron (и в соответствии с настройками)
1. При запуске cron Idea Hub забирает данные из каталогов контуров, которые указываются (при создании контура в интерфейсе).
