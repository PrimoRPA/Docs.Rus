...
1. Скопируйте `.env.example` в `.env` и пропишите в нем необходимые настройки. Настройки описаны в файле `.env.example`.
...
1. Запустите скрипт, указав в качестве аргумента имя окружения, например prod (от production):
   ...
   .\get_data.ps1 prod
   ```
1. Результат работы скрипта в виде файлов формата CSV будет помещен в каталог c именем окружения (prod). Каталог будет создан по пути, описанном в конфигурационном файле `.env` в параметре **OUTPUT_FOLDER**.

   В общем случае это:  `/var/www/IdeaHub/private/import-source/sync`.
	
1. Перенесите файлы, через удобный для вас носитель, на сервер Idea Hub в каталог `/private/import-source/sync/prod`.
1. После чего файлы можно будет импортировать в IH либо в автоматическом режиме с использованием утилиты cron, либо вручную со страницы настройки контура.


### Параметры скрипта get_data.ps1 

Все параметры скрипта `get_data.ps1` указаны в файле `.env.example`. С описанием параметров можно ознакомиться в этом разделе либо в самом файле.
