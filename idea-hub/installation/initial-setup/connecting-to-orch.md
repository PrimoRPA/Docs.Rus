...
Подключение к Оркестратору позволяет получить информацию о:
 ...
 - очередях, статистику обработки элементов — дата, средняя длительность, общее количество обработанных элементов, количество успешно обработанных элементов, количество элементов, обработка которых завершилась с бизнес-ошибкой или технической ошибкой;
...
1. Idea Hub запускается по cron, забирает данные из каталогов контуров (контуры и время начала импорта указываются при создании контура в интерфейсе). Время должно быть позже. 2 расписания: 1-е крон, 2-е в идеа хабе (импорт)
...
* Пользователь, от имени которого будут выполняться запросы, должен иметь права на чтение следующих баз данных — именно к ним будет обращаться скрипт:  
  * `ltools` - указать какие таблицы (только на чтение/селект)
  * `ltoolslicense` - указать какие таблицы (только на чтение/селект)
...
1. Проверьте и настройте все переменные в файле `.env`. Окружения или контуры — это экземпляры Оркестратора. Если используется несколько контуров, переменные в блоке для контуров (Environments) необходимо продублировать, изменив имена и значения для необходимых контуров. 
...
## Параметры скрипта get_data.sh (все параметры описаны в файле .env.example)
...
- `RPA_PROJECT_LAUNCHES_INTERVAL=5` — интервал в днях, за который будут получаться данные из таблицы RpaProjectLaunches. Позволяет получить статистику проектов, даже если отсутствовало подключение к БД Оркестратора в течение нескольких дней. Например, 25-го янв получили данные, 26-28-го янв подключения не было, данный (дописать).
- `RPA_PROJECT_LAUNCHES_FIRST_TIME_INTERVAL=300` — интервал в днях, который определяет дату, с которой мы в первый раз пытаемся посмотреть таблицу RpaProjectLaunches. (подумать над формулировкой)
...
* DB\_[VAR]\_[DB_NAME]_[ENVIRONMENT]=reader — ... VAR - креда; reader - значение составного параметра 
