...
* Пользователь, от имени которого будут выполняться запросы, должен иметь права на чтение следующих баз данных — именно к ним будет обращаться скрипт:  
  * `ltools` — в этой базе данных должны пользователю быть выданы разрешения для выполнения операции SELECT по отношению к следующим таблицам:
    ```
    ...
    ```
  * `ltoolslicense` — в этой базе данных должны пользователю быть выданы разрешения для выполнения операции SELECT по отношению к следующим таблицам:
    ```
