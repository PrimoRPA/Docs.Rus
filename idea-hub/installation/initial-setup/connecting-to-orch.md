...
Следующий шаг — перейти в веб-интерфейс Idea Hub, чтобы создать контуры организации и указать для них каталоги, из которых Idea Hub следует забирать данные Оркестратора, полученные с помощью скрипта `get_data.sh`.
