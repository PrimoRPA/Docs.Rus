...
Скрипты `get_data.sh` / `get_data.ps1` могут запускаться на любой машине, но файлы, которые они создадут, должны быть обязательно доступны на сервере Idea Hub.
...
* Пользователь, от имени которого будут выполняться запросы, должен иметь права только на **чтение** баз данных Оркестратора, к которым будет обращаться скрипт. Потребуется доступ к следующим базам и таблицам:  
  * `ltools`:
    ```
...
1. Скрипт запускается из каталога Idea Hub с аргументом — именем контура. Например:
   ```
   ./scripts/orc-data-fetch/get_data.sh prod
   ./scripts/orc-data-fetch/get_data.sh test 
   ``` 
1. Запуск скрипта отдельно для каждого контура добавьте в crontab. Пример строки crontab, которая описывает запуск скриптов каждый день 1 раз в 3 часа 10 минут:
   ```
...
Все параметры скрипта указаны в файле `.env.example`. С описанием параметров можно ознакомиться в этом разделе либо в самом файле.
...
1. Загрузите в этот каталог архив `orc-data-fetch.25.2.1.windows.tar.gz`. 
1. Перейдите в этот каталог и выполните команду:
	  ```
...
1. Скопируйте `.env.example` в `.env` и пропишите в нем необходимые настройки. Настройки описаны в файле `.env.example` с помощью комментариев.
...
1. Откройте PowerShell и перейдите в каталог со скриптом.
1. Запустите скрипт, указав в качестве аргумента имя окружения, например prod (от production):
   ```
   .\get_data.ps1 prod
   ```
1. Результат работы скрипта в виде файлов формата CSV будет помещен в каталог c именем окружения (prod). Каталог будет создан по пути, описанном в конфигурационном файле `.env` в параметре **OUTPUT_FOLDER**. В общем случае это:  `/var/www/IdeaHub/private/import-source/sync`.	
1. Перенесите файлы, через удобный для вас носитель, на сервер Idea Hub в каталог `/private/import-source/sync/prod`.
1. После чего файлы можно будет импортировать в Idea Hub в автоматическом режиме с использованием утилиты cron, либо вручную со страницы настройки контура.
