...
Ниже указаны требования к машине, на которой будет запускаться скрипт `get_data.ps1`. Машина должна работать под OC Windows.
...
* `.env.example` — пример того, как может быть оформлен файл контура. Содержит информацию о параметрах скриптов.
* `.env.message` — служебный файл, который обеспечивает работу скриптов. Не требует действий со стороны пользователя.
* `get_data.sh` — скрипт для запуска, который создаёт на машине каталоги контуров и складывает в них полученные данные из БД PostgreSQL Оркестратора.
* `get_data.ps1` — скрипт для запуска на Windows-сервере, который получает данные из БД MSSQL Оркестратора.
