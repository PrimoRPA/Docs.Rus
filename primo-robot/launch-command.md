...
* **aliveinterval:** управляет интервалом отправки запросов KeepAlive от Робота к Оркестратору. Запрос KeepAlive используется для сбора состояний Роботов: Робот отдает свое состояние Оркестратору одновременно с удержанием лицензии (так Оркестратор понимает, что Робот еще активен). Значение аргумента указывается в миллисекундах, значение интервала по умолчанию 10000 мс. Пример: `aliveinterval=20000`
