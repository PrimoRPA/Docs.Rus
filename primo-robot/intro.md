...
Работать с Роботом можно по-разному:

1. Автоматически через Оркестратор.
2. Вручную на компьютере пользователя через приложение **Primo Robot**. В этом случае робот выполняет функцию цифрового ассистента.
