...
1. **Наименование (Name)***  —  Название задачи. Используется для идентификации задачи в интерфейсе.
...
3. **Расположение (Location)**  — Указывает расположение проекта:  
   ...
   - **Orchestrator** — выполнение задачи на удаленном сервере.
...
1. **Вид окна (Startup position)**
   Определяет, в каком состоянии программа будет запускаться:  
...
2. **От имени администратора (As Administrator)** 
   Если этот флажок установлен, программа будет запускаться с повышенными привилегиями (правами администратора). Это требуется для выполнения задач, которые затрагивают системные настройки или ресурсы.
...
3. **Логировать в файл (Log to file)** 
   Если включено, логи автоматически сохраняются в файл. Файл логов хранится в директории, заданной в настройках программы. По умолчанию записываются по пути C:\Users\имя\AppData\Local\Primo\Log
...
1. **Без Оркестратора (No Orchestrator)** 
   Если установлен чекбокс, робот работает автономно, без взаимодействия с Оркестратором.
...
4. **Тенант (Tenant)**  
   Поле для указания идентификатора тенанта в Оркестраторе.
