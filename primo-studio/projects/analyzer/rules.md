...
| ST-PR-006  | Вложенные блоки if                | Процесс      | Предупреждение  | В процессе превышен максимально допустимый порог вложенности для элемента If-Else. Рекомендуется уменьшить вложенность блоков if до минимума, чтобы легче было поддерживать процесс. Правило имеет параметр "Порог", значение по умолчанию `3` |
...
Вы можете разработать собственные правила анализа кода согласно [этой инструкции](https://docs.primo-rpa.ru/primo-rpa/developers/ltools.sdk/rules).
