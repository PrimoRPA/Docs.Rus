...
Для корректного поиска приложения достаточно заполнить одно из полей[^1]:
...
[^1]: Второе поле можно оставить пустым. Если заполнены оба поля, будут учитываться оба значения.
