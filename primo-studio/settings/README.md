...
4. Укажите оставшиеся параметры для выбранной AI. 
   ...
   >	:small_blue_diamond: ***О том, как получить токен для работы с GPT, ClientSecret или Folder ID, см. [здесь](https://docs.primo-rpa.ru/primo-rpa/primo-studio/settingsAI).***
   
   Sber GPT:
   * **Token** - токен для работы с GPT. 
   * **Key** - ваш ClientSecret.
   * **Time-out** - предельное время ожидания ответа GigaChat в миллисекундах. По умолчанию `20000` (20 сек). 
   * **Creativity** - творческая составляющая ответа. Число от 0 до 1. По умолчанию `0.5`.
