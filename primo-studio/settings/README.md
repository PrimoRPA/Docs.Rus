...
18. **Отладчик чистого кода** - позволяет выбрать ядро робота-отладчика: `v1` или `v2`. Параметр добавлен в версии 1.23.11.  
    ...
    * Вместо кода для точки останова использовать цветовые индикаторы (см. на рисунке ниже). 

    ![](<../../.gitbook/assets1/set-breakpoint.png>) 

    * Просматривать на остановках текущие значения локальных переменных функции. В версии v1, в панели «Вывод», видны только текущие значения переменных класса.
    * Переходить к коду функции на остановках.
    * Удалять все имеющиеся в процессе точки останова по кнопке **Убрать все точки останова** на вкладке «Отладка».
...
* [Ручная установка расширений и плагинов из Студии](https://docs.primo-rpa.ru/primo-rpa/primo-studio/settings/plugin-install#ruchnaya-ustanovka-iz-studii);
...
3. Перейдите в настройки, включите чекбокс **Использовать AI** и выберите нейросеть:
   * Sber GPT - установлена по умолчанию.
   * Yandex GPT.
...
4. Укажите оставшиеся параметры для выбранной AI. Информацию о том, как получить токен для работы с AI и Folder ID, см. [здесь](https://docs.primo-rpa.ru/primo-rpa/primo-studio/settingsAI).

   GigaChat:
   * **Token** - токен для работы с GPT. 
   * **Key** - ?
   * **Time-out** - предельное время ожидания.. (получения токена?).. в миллисекундах. По умолчанию `20000` (20 сек). 
   * **Creativity** - число от 0 до 1. По умолчанию `0.5`.

   Yandex GPT:
   * **Folder ID** - идентификатор папки... 
   * **Token** - токен для работы с GPT. 
   * **Time-out** - предельное время ожидания.. чего именно?.. в миллисекундах. По умолчанию `20000` (20 сек).
   * **Max Lenght** - максимальная длина (токена?). По умолчанию `4000`.
   * **Creativity** - число от 0 до 1. По умолчанию `0.5`.
