...
3. В открывшейся форме:
   * В названии проекта обязательно укажите префикс **Primo.** Пример: `Primo.My.Activity`.
...
1. Вызовите контекстное меню папки Elements, выберите **Добавить > Создать элемент > Класс C# Class**.  Укажите имя класса, например, `MyFirstActivity.cs`. Данный класс будет содержать программную часть (code-behind) элемента Primo, описывать его логику.
