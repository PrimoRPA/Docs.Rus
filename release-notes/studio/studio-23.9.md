...
1. 3199 - Улучшено поведение элемента [If-Else](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_logic/el_logic_ifelse) в режиме отладки. Если на панели элемента была включена точка останова, то при паузе на отладке требовалось дважды жать «Следующий шаг» , чтобы возобновить процесс. Сейчас выполнение возобновляется с первого раза. 
2) При пошаговой отладке if-else иногда может зацикливать, а также неверно выбирать ветвь (всегда "истина").
Это не всегда воспроизводится, но точно воспроизводится 1)
...
1. 3271 - Устранена утечка памяти при закрытии процесса. 
