...
1. Изменился внешний вид Редактора шаблона и логика его работы:
   * Добавилась возможность отобразить дерево компонентов. И-и-и.... что  с ним можно сделать?
   * Дерево возможно обновлять
   * Появился выбор доступных переменных (где именно?) - было бы здорово, когда пишем фигурные скобки, чтобы передать переменную (н-р, в поле "Текст" в контейнере с "Присоединиться к браузеру") - чтобы примо предлагала на выбор доступные переменные или даже писала код С#. Например, {(1+1).ToString()}. С передачей переменной все работает.
   * "Нажал на валидацию и после возвращение в примо висит выделенный элемент. Чтобы его выключить нужно нажать на какое-нибудь поле или закрыть шаблон поиска. Как-то не удобно" - как именно это исправлено?
   * Многоуровневый селектор. Например, при указании тегов, как это было в старом селекторе - сделано или нет?
