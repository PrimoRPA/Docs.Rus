...
1. Значительно улучшен UI/UX **Редактора шаблона**:

   * В окне инструмента **Выбор элемента** (Picker) появились подсказки с описанием горячих клавиш. 
  
     ![](<../../.gitbook/assets/picker-3.png>)
   
   * При формировании шаблона поиска выводятся подсказки, помогающие скорректировать выбор свойств. Кроме этого появилась возможность редактировать значения свойств прямо в редакторе.
  
     ![](<../../.gitbook/assets/tips-in-template-editor.png>)
     
   * При нажатии кнопки **Валидация настроек шаблона** запускается проверка текущих настроек, а результат отображается внизу редактора. Найденный элемент управления по-прежнему подсвечивается.

     ![](<../../.gitbook/assets/search settings validation.png>)

   * Добавлены вкладки **Структура** и **Код** (шаблон поиска в JSON), а также возможность быстро поменять селектор.

     ![](<../../.gitbook/assets/structure-tub-in-editor.png>)

   * Сохранились возможности выбора режима (Контейнер, Браузер, Рабочий стол, SAP) и возможность перевыбрать элемент управления. Добавилась опция **Путь до элемента**, позволяющая <управлять родительскими и дочерними селекторами>.
