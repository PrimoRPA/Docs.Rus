...
1. Изменился интерфейс редактора шаблона поиска (селектора). (Сергей Гуляев).
...
1. В контейнере [Приложение Excel](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_excel/el_excel_app) появилось свойство **Открывать только на чтение**. книга если скачана из интернета.
...
1. Исправлен импорт следующих SAP-элементов из UIPath-проекта:
   * **Type into** > теперь конвертируется как один элемент **Ввод текста** (LTools.UIInteraction.Elements.WFTypeText) с корректными свойствами и режимом SIMULATE.
