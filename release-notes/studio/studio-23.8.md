...
1. 1598 - Не считывает Asset из Оркестратора в Object. 
у активности WFGetAsset ("Получить значение")  в поле "Результат" (тип Object) не "подтягивались" данные из ассета (ресурса) в Оркестраторе, если у ассета был тип данных не "String", а какой-нибудь другой.
Согласно комментарию разработчика, был изменен принцип работы поля "Результат" активности WFGetAsset ("Получить значение").
Теперь в поле "Результат" (тип Object) в активности WFGetAsset ("Получить значение") подтягиваются данные ассета (ресурса) из Оркестратора почти с любым типом.
///
