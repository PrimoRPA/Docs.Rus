...
1. Ввести понятие атомарность/транзакция в процессы:
   * при переносе нескольких элементов а затем отмене действия, элементы будут возвращаться на изначальное место по одному а не все разом
   * Если Вырезать-Вставить элемент, а затем отменить это действие, то элемент останется в двух позициях. В итоге, для того чтобы отменить действие нужно 3 раза нажать "Отменить".
Итог: Элемент совершает необходимое действие за одно нажатие
...
1. Ввести понятие атомарность/транзакция в процессы https://azure-dos.s1.primo1.orch/PrimoCollection/Studio/_boards/board/t/Studio%20Team/Stories/?workitem=8519
