...
1. Расширены возможности возможности работы с JSON. Теперь поддерживается автоматическая десериализация не только в тип `Dictionary`, но и в различные типы коллекций, включая `List<T>`.
1. Улучшена работа шаблона поиска (селектора) при взаимодействии с Java-элементами через плагин **Java_ext**. Теперь плагин обеспечивает точное формирование селекторов как с применением, так и без использования свойства **Text**.
...
1. Для элемента **Ссылка на процесс** добавлена функция подсветки изменений, внесенных в аргументы подпроцесса. Теперь, когда пользователь вносит изменения в аргументы подпроцесса, это будет подсвечено. Подсветка исчезает после того, как произведено редактирование и назначение переменных.
