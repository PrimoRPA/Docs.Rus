...
1. В процессе с типом **Последовательность** исправлена ошибка, когда при перемещении нескольких элементов комбинацией `Ctrl` + левая кнопка мыши переносился только один элемент. Теперь пользователи могут легко перемещать несколько элементов, удерживая `Ctrl`. 
