...
----
1. У элемента [**Открыть браузер**](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_browser/el_browser_open) появилось свойство **Состояние загрузки**, отвечающее за ожидаемый статус загрузки веб-страницы, указанный в **Открыть браузер**. Возможные значения:
   * *None* - не ждать загрузки страницы. Значение используется для обратной совместимости и установлено по умолчанию. 
   * *Interactive* - ждать загрузки страницы (и построения DOM-дерева), но дополнительные ресурсы, такие как изображения и iframe (en-US), могут еще загружаться. (ждет джава скрипт?)
   * *Complete* - ждать полной загрузки страницы.

   В результате переход к первому элементу контейнера будет осуществлен только после достижения установленного состояния загрузки. Ранее действия, помещенные в контейнер "Открыть браузер" не дожидались открытия браузера. В результате чего приходилось увеличивать таймаут ожидания, например.
...
Сделал, но посредством URL. Соответственно, URL разные должны быть. https://disk.yandex.ru/d/WK5OxnyhtPpLlA   Нужно переустановить расширение!!!
...
1. Исправлена ошибка, из-за которой робот-отладчик не мог выйти из подпроцесса, если в элементе **Ссылка на процесс** не было включено кеширование. Ошибка воспроизводилась при наличии в подпроцессе элемента **Try-Catch**, в блоке **Try** которого возникало исключение. С этой версии установка/отсутствие кеширования подпроцесса не влияет на его работу.





1. 4141	"Присутствие элемента" браузера Chrome в цикле работает не корректно, ошибки работы...		Alexandr Kazeko
1. 7808	Некорректное поведение элементов "МойОфис" (Офис)						Alexandr Kazeko
1. 9337	Активность "Read table" (не 1 поколения) не способна считать большие таблицы			Alexey Perkin
