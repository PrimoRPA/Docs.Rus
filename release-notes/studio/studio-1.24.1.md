...
1. У элемента [**Открыть браузер**](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_browser/el_browser_open) появилось свойство **Состояние загрузки**, определяющее ожидаемый статус загрузки веб-страницы. Возможные значения:
   ...
   * *Complete* - ждать полной загрузки страницы.
   ...
   В результате переход к первому элементу контейнера будет осуществлен только после достижения установленного состояния загрузки. Ранее действия, помещенные в контейнер "Открыть браузер" не дожидались открытия браузера. 
