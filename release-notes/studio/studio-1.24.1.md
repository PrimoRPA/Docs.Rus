...
1. Исправлена ошибка, из-за которой не открывался ltw-файл в **Ссылке на процесс**. ..если какой-то процесс найти через поиск (CTRL+F) и открыть его, то он не открывается и не показываются его аргументы. Теперь открытие через происк происходит корректно.
1. Исправлена ошибка при совпадении имени переменной и аргумента в процессе и подпроцессе. ...если в нашем процессе создана какая-то переменная и в подпроцессе, на который ссылается **Ссылка на процесс**, был создан аргумент с тем же именем, но с другим типом. Анализатор ругался на несовпадение типов. Проблема была в том, что аргументы и коллекции лежали в одной коллекции и поиск шел по имени. 
