...
### События (казеко)
1. Работоспособность элементов типа "Событие" (Браузер). Не работали ВСЕ элементы типа "Браузер > События", надо было починить:
   * Клик элемента
   * Событие изменения аттрибута
   * Событие кнопки браузера
1. Присвоение переменных в параллельных процессах приводит к ThrowCrossThreadException.
