...
1. Устранена ошибка в работе событий, из-за которой робот внепланово завершал выполнение проекта. Ошибка возникала при установке параллельного режима в контейнере **События** и попытке присвоить переменные в параллельных потоках. 
1. Исправлена некорректная работа элементов **Удаление колонок** (Excel) и **Удаление строк** (Excel) в чистом коде, связанная с ошибкой методов.
...
### Зависимости
1. Устранена утечка памяти при работе с зависимостями. Теперь при запуске проекта, в папке , будет создаваться папка типа GUID с зависимостью `AppData/Local/Temp` пользователя стала очищаться.... при запуске проекта создается папка типа GUID с зависимостями, которая удаляется в одном из следующих случаев:
