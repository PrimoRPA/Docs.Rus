...
1. Улучшена работа с зависимостями. Добавлена очистка папки `AppData/Local/Temp` пользователя: теперь при запуске проекта создается папка типа GUID с зависимостями, которая удаляется в одном из следующих случаев:
