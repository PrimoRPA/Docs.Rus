...
5. (830/837) Возможность организации технической паузы для машины роботов. Исправление бага с постоянно растущим ID РДП-сессии. Сюда же задача (907) - Фиксация периодов "простоя" машины роботов

Суть ошибки: Счетчик включенных сессий мог доходить до большого числа (13к). Заметили, что 13к - это предел, после которого новые сессии на машине агента не создаются, ее нужно перезагружать. Решение: для того, чтобы скинуть счетчики сессий (подключенных по RDP пользователей), нужно делать паузу между подключениями. 
Там где счетчик забивался - это значило, что постоянно шли новые подключения, и не было на агента такого момента, что нет роботов вообще (всегда либо роботы, либо учетки через которые разрабы сидят, что-то запускают). ПОэтому счетчик не мог сброситься.
Решение: для машин роботов устраивать паузы небольшие (около минуты-полутора). Если машина столько простоит без роботов, то счетчик будет старые значения похватывать = не будет постоянно расти.  
Где ставить паузу: в UI в разделе "Машины роботов" появилась кнопка "Перерыв" (она к выбранной машине относится или ко всему списку машин?). В параметре задаем паузу (по умолчанию 60 сек.). Пауза работает так: как только на машине не будет работать ни одного робота (последний выйдет), начнется пауза. Во время паузы нельзя будет подключиться к ней (к рдп-сессии?). За время пауы сбросится счетчик и сл подключение будет уже "с нормальным номером" (что такое нормальный номер?). Блокировка = не подключается сервис РДП к машине робота. Увидит, что она на паузе. Робот не получит рдп-сесиию = не стартанет. Главное, не выставлять большой интервал для паузы - который будет перекрывать указанное время в конфиге, для попыток получения сессии (оно должно быть больше). Если же выйдем за интервал, то в логах вебапи будет ошибка, что за такое-то кол-во попыток Орк не смог открыть рдп-сессии. В конфиге кстати можно отключить кнопку "Перерыв", чтобы не показывать ее в UI.
Все это происходит на стороне Оркестратора: все проверки, в том числе на возможность получения рдп-сессии для выбранного робота. 
Паузы просматриваются в БД (это на сайте писать не надо, но дока по БД д.б. обновлена). 
