...
1. Добавлена рассылка "Все события Оркестратора" (по событиям с типом "Все" - рассылка идет не прям по всем событиям, а только по заданным. список событий настраивается в конфигурационном файле службы States - в секции `RabbitMQ`, в параметре `AllEventsResolveList`. Указываются коды событий.
