...
1. Расширены возможности для работы с формулами:
   * Добавлена возможность указывать переменные. Например, переменной можно присвоить значение каких-либо обработанных элементов очередей для вычислений.
   * Добавлена возможность комментировать строки с помощью синтаксиса `//Комментарий к формуле`. Чтобы потом было проще ориентироваться.
   * Появилась возможность использовать справку при написании формул:
     * `...@F` — позволяет просмотреть список всех доступных пользователю функций. **Уточнить про регистр и конечный синтаксис у Кости**
     * `@E` — позволяет просмотреть список доступных пользователю сущностей. Например, очереди, лицензии, процессы и т.д.
