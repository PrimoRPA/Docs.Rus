...
реализовали ежедневный сбор статистики запуска проектов и обработки элементов очередей. Теперь статистика датирована и может быть автоматически дозаписана задним числом.

Суть: из-за того, что теперь мы имеем ежедневную статистику, мы можем считать эффекты в разрезе любого интервала времени. Дата - это только число, без времени, данные из Орка за определенный день. 

реальная запись может произойти позже в том числе за какую-то дату. 

раньше статистика усреднялась.

FTE считается только за один текущий день, за сегодняшний - конкретная дата. Когда за период или интервал смотрим FTE - то 
