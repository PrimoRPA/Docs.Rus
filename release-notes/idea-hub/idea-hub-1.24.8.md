...
### Улучшенные формулы
1. Добавлена поддержка переменных в полях формул:
   * Теперь легко можно определить переменные, которые вы хотите использовать в расчетах.
   * Использование переменных упрощает написание сложных формул и повышает их читаемость. 
1. Добавлена возможность писать комментарии в формулах. Функция позволяет упростить понимание и поддержку сложных формул. Чтобы добавить комментарий, используйте синтаксис `//Комментарий к формуле`. Комментарии игнорируются при выполнении формулы, но остаются видимыми для всех, кто с ней работает.


1. Стали доступны подсказки при вводе формул. 
 


3. Добавлена возможность оставлять комментарии с помощью синтаксиса `//Комментарий к формуле`. Комментарии помогут ориентироваться в сложной формуле.
4. Добавлена динамическая справочная информация, которая поможет написать формулу:
     * `@F` — просмотр списка всех доступных пользователю функций. **Уточнить про регистр и конечный синтаксис у Кости**
     * `@E` — просмотр списка доступных пользователю сущностей. Например, очереди, лицензии, процессы и т.д.
   * Автоматическую справку можно дополнять текстом (=можно дополнить подсказки). На страницах ИХ можно создавать страницы с произвольным текстом, и этот текст мы можем использовать в качестве справки, добавлять в разных ситуациях. (если мы создаем материал Статья, напишем там какой угодно текст отформатированный (может html использоваться). Эти статьи мы можем привязывать к подсказкам - например, чтобы к process привязать статью, в коде есть крючки.... какие конкретно крючки и как привязывать, не знаю.
