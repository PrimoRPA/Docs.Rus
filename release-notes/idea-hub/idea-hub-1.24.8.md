...
* теперь вы можете просмотреть эффект в разрезе любого интервала времени, поскольку вся статистика датируется.
...
1. Добавили поддержку переменных в полях формул. Теперь легко можно определить переменные, которые вы хотите использовать в расчетах. Использование переменных упрощает написание сложных формул и повышает их читаемость. 
...
1. При вводе формул стали доступны подсказки. Система автоматически отобразит подсказки, которые относятся к указанному значению. Кроме того, стали доступны операторы:
...
1. версии проекта не хранятся в ИХ. теперь в статистике отображаются общие данные по запускам, без привязки к версиям (общее количество запусков, кол-во успешных запусков, кол-во запусков с ошибками, кол-во запусков с исключениями). таблица со статой по запускам проектов тоже обрела новый вид (скрин мб?) Проблема с версиями была в сл: совокупность данных о проекте невероятно огромное из-за версий в том числе мб, поэтому оптимизировали для чистоты данных (это не надо писать). 
...
4. Добавился просмотр статистики в разрезе дня или периода.
...
1. Например, привязать новый проект. И тогда ИХ выдаст совокупную стату в процессе по обоим проектам. 
1. Или привязать новый проект к новому процессу.
