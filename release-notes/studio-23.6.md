...
1. Добавлено опциональное свойство **Занимать** для элемента [**Получить из очереди по ID**](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_orch/els_queues/peekqueueid). Параметр определяет, нужно ли возвращать в ответе только те элементы очереди, которые можно сразу занять - изъять из очереди, чтобы с ними начал работу ваш Робот. По умолчанию параметр выключен.
