...
### ФИЧИ - НЕ ОПИСАНЫ
1. Добавлена функциональность **Общие папки** - наличие разных папок позволяет разграничить доступ пользователей к объектам Оркестратора. Например, к RPA-проектам, роботам, заданиям и др. По умолчанию функция отключена, [разрешить ее использование](https://github.com/PrimoRPA/Docs.Rus/blob/1114-%D1%80%D0%B5%D0%BB%D0%B8%D0%B7-%D0%BE%D1%80%D0%BA%D0%B5%D1%81%D1%82%D1%80%D0%B0%D1%82%D0%BE%D1%80%D0%B0-2401/orchestrator/deployment/fine-tuning/shared-folders.md) можно в конфигурационном файле.

   Среди папок выделяются корневая и не корневые. Корневая папка выдается только одна на тенант, у нее нет владельца, и все пользователи по умолчанию имеют к ней доступ. У не корневой папки есть владелец - это пользователь, создавший папку. Он имеет неограниченные права на папку, а также может делегировать права другим пользователям.

   Никакой объект не может находиться одновременно в корневой папке и в какой-либо не корневой папке. На текущий момент папки не работают с такими объектами, как ресурсы и очереди обмена данными.
