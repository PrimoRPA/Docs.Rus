...
1. Добавлена возможность скрывать из интерфейса кнопку **Консоль**, которая открывает окно с видеотрансляцией RDP-сессии робота. Скрыть функцию можно в настройках конфигурационного файла WebApi. Для этого в секции **Worker** установите параметру **ShowRdpConsoleBtn** значение `false`:
   ```
   "Worker": {
...
1. В [форме добавления проекта](https://docs.primo-rpa.ru/primo-rpa/orchestrator/basics/add-project#parametry-proekta) появились параметры **Версия** и **Предрелиз (RC)**. Они позволяют указать, является ли загружаемая версия предрелизной. В таблице проектов указанная информация будет отображаться в столбце **Версия**.

   ![](<../../.gitbook/assets1/orch-project-rc-parametr.png>)

1. Для ролей добавлена возможность настроить права доступа к функции **Предрелизная версия**, которое находится в группе **RPA-проекты**. В разделе настроек прав роли добавлена возможность управлять доступом к функции предрелизной версии RPA-проекта. С ее помощью можно настроить права на ... . Данная опция необходима для тех случаев, когда определенным лицам необходимо протестировать последнюю версию RPA-проекта перед ее основным релизом. С этой опцией Видны список всех версий проекта, остальным у кого нет - последняя версия не видна. Данное правило распространяется и на роботраннер (аттендед робот).
upd:
не отобржение, а запрет пользоваться
