...
1. В форму создания ресурса добавлены опциональные параметры **Тип блокировки** и **Таймаут**. Установка блокировки гарантирует, что другие роботы не смогут обратиться к данному ресурсу. Таким образом, обрабатывать ресурс может только робот-владелец. Возможные типы блокировки:
   * *Разрешено чтение* - другие роботы могут только читать этот ресурс.
   * *Запрещено чтение* - другим роботам полностью запрещено использовать этот ресурс.

1. Добавлена возможность поиска элемента очереди по натуральному ключа без учета регистра.
...
### ФИЧИ - НЕ ОПИСАНЫ
1. Функционал должен позволять работать с проектом в IdeaHub - Студия - Оркестратор. Те проект для всех систем должен быть некой идентичной сущностью с доступными свойствами.
...
## Исправленные ошибки
1. Исправлена ошибка, когда на запрос элемента очереди по FIFO возвращался `Null`, хотя очередь не была пустой. Ошибка возникала при большой нагрузке и требовала от пользователя лишних действий в виде повторного обращения к очереди. Теперь необходимость повторных извлечений устранена. 
...
1. Исправлена ошибка, из-за которой в форме расписания невозможно было указать календарь, если их несколько (включен режим множественных календарей).
...
1. Исправлены ошибки в авторизации attended-робота и в отправке его логов в Оркестратор. 
...
1. Hope UI. Исправлена ошибка с отображением данных в разделе **Аргументы проекта > История запусков**.
