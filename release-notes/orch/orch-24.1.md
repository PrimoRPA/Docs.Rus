...
1. Понижен уровень лога *Робот не в состоянии мягкого останова* - теперь он является информационным. Ранее был предупреждением.
...
1. (372) Исправлена работа функции **Повторить** для элементов очереди обмена данными. (обнуление элемента, пересчет статистики).
1. (9299) Триггер по расписанию с мягкой остановкой не работает.
1. (10423) Исправлена ошибка, из-за которой триггер на изменение папки работает только со стратегией "Прикрепленные роботы" (настройки стратегии брались только из дефолтного тенанта).
...
1. Исправлена ошибка с кэшированием робота в методах работы с ассетами.
...
1. (11209) Исправлена ошибка в миграции для MSSQL с циклами в каскаде.
1. (10422) Исправлена ошибка, из-за которой машины агентов переносятся между папками без RDP-пользователей.
1. (10419) При переносе RPA-проектов между папками, промежуточные версии проектов теряются (перенос всех версий).
1. Исправлена ошибка в блоке "Аргументы проекта" > История запусков.
...
1. (10328) Исправлена ошибка с отсутствием сообщения о не последней версии проекта.
