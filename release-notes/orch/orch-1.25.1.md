...
1. Добавлен чекбокс **Уникальный запуск**.  
   - Чекбокс отображается рядом с полем ***"Кол-во одновременно работающих роботов"***, если значение этого поля больше `1`.  
   - Если значение поля равно `1` или не задано, чекбокс скрыт.  
   - Позволяет настроить уникальное выполнение заданий роботами при параллельной работе. 
