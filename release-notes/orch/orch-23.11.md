...
1. В разделе **Настройки > Email входящей почты** добавилась возможность выбрать **MS Exchange** в качестве протокола. Если он выбран, то отобразится поле **EWSUrl** - адрес сервера входящей почты. Также, для удобства пользователей, была оптимизирована таблица почтовых адресов: теперь вместо нескольких колонок с протоколами и портами выводится одна - **Протокол**. При наведении курсора на значение колонки можно увидеть параметры протокола: например, имя сервера и порт.
...
1. В таблице машин роботов стало легче определить доступность машины. Для этого в колонку **Доступна** были добавлены:
   * Подсветка значений. Так, если машина доступна, то значение подсветится зеленым. Если недоступна - красным.
...
1. Исправлена ошибка в постраничной навигации в журнале робота и в разделе **Мониторинг**. Теперь переход по страницам осуществляется корректно. 
