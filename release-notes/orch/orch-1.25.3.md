...
1. Устранена ошибка, возникавшая при попытке повторного запуска и соединения Студии с Оркестратором и выдававшая сообщение "Error connecting to Orchestrator...Status code - Conflict". В ходе решения данной задачи из конфигурационного файла appsettings.json был удален параметр `IntervalForReleaseInSeconds` для секции Studio; теперь его значение по умолчанию - 120 секунд.
