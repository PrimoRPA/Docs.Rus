...
1. Реализована и подключена к службам Agent и MachineInfo настраиваемая подсистема мониторинга служб, настройка которой осуществляется в разделе Monitoring файла конфигурации. В качестве внешней службы сбора данных может использоваться Zabbix или Prometheus, использован может быть любой доступный порт старше 1024; также может быть включен мониторинг .NET с помощью параметра DotNetMonitoringEnabled. Пример:
```
"Monitoring": {
