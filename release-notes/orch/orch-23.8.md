...
1. только для Postgres??? - Оптимизирована работа Роботов с очередью обмена данными при чтении элементов. Появилась возможность настроить тип реакции на блокировку элемента, возникающую при извлечении элемента из очереди по FIFO. Тип реакции устанавливается либо в конфигурационном файле WebApi в секции ExchangeQueue - глобально для всех очередей, либо на уровне отдельной очереди в UI Оркестратора. Для этого в UI, в разделе создания/обновления очереди, добавлены параметры:
   * **Реакция на невозможность извлечения элемента по FIFO** - определяет, что нужно вернуть Роботу (при блокировке?): null или ошибку. Значение **Вернуть ошибку** рекомендуется при обработке заранее подготовленной очереди, с которой работают только Роботы-читатели. Это позволяет отличать блокировку от случая, когда в очереди закончились элементы. В этом случае null будет являться признаком завершения (работы с очередью?).\
     При использовании динамической очереди, когда с ней одновременно работают Роботы-писатели и читатели, рекомендуется выбирать значение **Вернуть null**. Тогда null будет возвращаться и при ошибке, и при пустой очереди, а сигналом завершения работы с очередью будет внешний признак (как он устанавливается, какие бывают?). Например, достижение максимального количества итераций, или получение внешнего сигнала (какого?). Этот режим работы является более высокопроизводительным и является предпочтительным при большой нагрузке.
   * **Кол-во попыток при извлечении элемента по FIFO** - применяется, если в качестве реакции на невозможность извлечения элемента по FIFO выбрано **Вернуть ошибку**. Может привести к увеличению времени извлечения элемента.

    Настройка в UI имеет приоритет над глобальной. 

Раньше всегда возрвщался null (ошибка или нет элементов). Под ошибкой понимается блокировка записи в БД (блокировалась очередь или элемент?). Сейчас 3 режима работы очереди:
1) null - возвращаем, если произошла ошибка, и если закончились элементы. высокопроизводительный режим.  
2) null - возвращаем, когда кончились элементы. и если произошла ошибка, то возвращаем ошибку. менее быстрый режим. рекомендуется, если нет высокой нагрузки (на сервер? на бд?)
3) новый режим - это режим работы с ....? 
режимы отличаются по перформансу.
