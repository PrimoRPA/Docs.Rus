...
1. В раздел RPA-проектов добавлены вкладки:
   ...
   * **Запуски** — содержит таблицу с историей запуска проектов. В нее попадают как завершенные проекты, так и находящиеся в ожидании либо в процессе работы. Чтобы просмотреть значения аргументов, с которыми был запущен проект, необходимо нажать кнопку **Показать детализацию запуска**.
