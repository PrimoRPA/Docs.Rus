...
1. В раздел RPA-проектов добавлены вкладки:
   * **Очередь проектов** — на вкладке отображается таблица с проектами, ожидающими выполнения роботом.
   * **Запуски** — на вкладке отображается таблица с историей запуска проектов. Проекты могут находиться в разных статусах: в ожидании, в процессе работы и завершенные. Чтобы просмотреть значения аргументов, с которыми был запущен проект, необходимо нажать кнопку **Показать детализацию запуска**.
