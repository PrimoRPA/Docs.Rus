...
1. Добавлена новая настройка **Открывать закрытые вкладки с точками останова** (**Open closed tabs with breakpoints** в англоязычной версии), которая позволяет автоматически открывать закрытые вкладки *.ltw-воркфлоу* с установленными точками останова во время отладки. Для включения настройки в Primo RPA Studio перейдите в меню **Файл** → **Настройки** → **Общие** → **Отладчик**. Установить галочку напротив опции **«Открывать закрытые вкладки с точками останова»**.
1. Добавлена возможность отображения часто используемых свойств напрямую на элементе. Введённые данные в параметрах активности автоматически синхронизируются с соответствующими свойствами элемента в правой части окна. 
Для отображения параметров активностей в процессах с типом Диаграмма необходимо дополнительно установить галочку `Отображать дизайн элементов диаграммы`. По умолчанию функция отключена. Для включения перейдите в Студию в раздел **Файл → Настройки → Общие → Элемент**ы, выберите версию `v2` и для процессов с типом "Диаграмма" установите галочку `Отображать дизайн элементов диаграммы`.
...
1. Добавлено свойство **Игнорировать спецполя** (Ignore special fields)** в активность **Получить из очереди по фильтру (PeekQueueByFilter)** для процессов **Только код (Pure Code)**. Свойство позволяет получать элементы очереди с игнорированием специальных полей. По умолчанию свойство отключено.
...
1. Обновлен архив со скриптами для **[автоустановки](https://docs.primo-rpa.ru/primo-rpa/primo-rpa-studio/settings/autoinstall-browser-extension)** расширения **Primo RPA Extension**, используемого для взаимодействия с браузерными элементами в **Primo RPA Studio**. Процесс установки стал проще:  
...
1. Улучшен поиск у окна настроек Студии. Теперь поиск осуществляется по тексту самих настроек и подсказок к ним.
1. Добавлена подсветка ошибок при использовании аргумента, удалённого из панели, для активностей «Присвоение», «Множественное присвоение», «Запись в журнал» и циклов, таких как «For Each» и «For Each для DataTable».
...
1. Исправлена ошибка в активности «Функция BAPI», из-за которой параметры из входного поля «Options» не использовались, что приводило к выполнению без параметров. Теперь параметры «Options» корректно учитываются.
...
1. Устранена ошибка в работе **Desktop Anywhere (Primo Remote Agent)**, из-за которой элементы "Поколения 1" не работали корректно при выборке контролов.
...
1. Исправлена ошибка, из-за которой робот зависал при возникновении ошибки в процессе типа "Только код" (OnlyCode) в режиме Debug. Теперь робот корректно завершает выполнение или останавливается в блоке try-catch без зависания. При остановке в процессе типа "Только код" (OnlyCode) в режиме Debug кнопка "Повторить шаг" стала недоступной, что предотвращает некорректное поведение.
1. Исправлена некорректная работа элемента "Переместить файл" в ситуациях, когда исходный файл недоступен. Если опция "Перезаписать" активирована, а файл-источник отсутствует, целевой файл больше не удаляется.
