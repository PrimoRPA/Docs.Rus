...
1) При закрытии Студии сбрасывалось положение окна редактора и установленный размер. Теперь настроенные параметры окна сохраняются.
2) Терялся фокус курсора при выборе значения из списка подсказок, который всплывает при написании кода, - курсор возвращался на первую строку. Ошибка исправлена.

1. При поиске переменных название активности "Множественное присвоение" не отображается https://dev.azure.com/primorpa/Primo/_queries/edit/892/?triage=true
