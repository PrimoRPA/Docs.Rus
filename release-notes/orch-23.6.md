...
1. Оптимизирована логика процессов, связанных с:
   ...
   * 1. Оптимизация подсчета запущенных роботов. Добавлен подсчет количества процессов роботов на Агенте - (2497) в чем суть, раньше неправильно считалось? (колонка "Макс. кол-во работающих роботов" )

dct jgnbvbpfwbb - xnj,s hfpuhepbnm <L? yt ,skj ,kjrbhjdjr b nl/
Оптимизации:
...
1. Исправлена ошибка с удалением используемого в очереди проектов задания. - если проект попал в очередь  значит там есть инфа из какого задания он папол. когда пытаемся удалить задание там сработает ограничение.
