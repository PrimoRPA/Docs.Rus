...
Тестовый запрос можно отправить через интерфейс AI Server — для этого существует специальная страница **Тестирование**. Пользователю потребуется выбрать тип модели и загрузить изображение, после чего результат отобразится на соседней вкладке. Все распознанные данные доступны для просмотра из интерфейса.
