...
Компонент AI Текст устанавливается на целевой машине — логическая сущность, которая должна состоять из двух физических или виртуальных машин. 
* Agent.NlpEngine — агент для управления контейнером с NLP-ядром.
* LogicsServer — приложение на Python, управляющее NLP-ядром (VLLM/LLAMA). Управляется основным агентом. LogicsServer поставляется в виде отдельного докер-образа.
