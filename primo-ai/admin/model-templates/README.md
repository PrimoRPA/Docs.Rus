...
* `base-LLM-01 (Vllm)` — базовая LLM-модель, которая работает на движке Vllm. Этот тип базовой модели обрабатывает запросы быстрее и точнее, но более требователен к аппаратным характеристикам целевой машины. Данную модель следует использовать на машине с графической картой или высокопроизводительном CPU.
