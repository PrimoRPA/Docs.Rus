...
:large_orange_diamond:***Важно**. Если машина выключена, то на ней невозможно будет запустить обучение и инференс. Такая машина столбце «Доступна» будет отображена со статусом `Выключена`. Спустя минуту после выключения машины освободится лицензия, которую занимает ее агент.*
