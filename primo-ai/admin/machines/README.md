...
Убедитесь в доступности целевой машины — если агент настроен корректно, то в столбце **Доступна** отобразится значение `Да` и время последнего опроса машины. Период опроса настраивается в конфигурационном файле Api в параметре **Agents > Period**.

Если машина недоступна, то в столбце отобразится значение `Нет`. На недоступной машине невозможно будет запустить процессы инференса и обучения.
...
:large_orange_diamond:***Важно**. Если машина выключена, то на ней невозможно будет запустить обучение и инференс. Спустя минуту после выключения машины освободится лицензия, которую занимает ее агент.*
