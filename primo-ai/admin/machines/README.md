...
Чтобы освободить лицензию, потребуется выключить целевую машину. Перед выключением машины необходимо убедиться, что на ней не запущены процессы модели OCR или модели NLP. 
...
По умолчанию все созданные машины включены. Как включить машину, если ранее она была выключена:
...
:large_orange_diamond:***Важно**. Спустя ~ минуту после выключения машины освободится лицензия, которую занимает ее агент. На выключенной машине невозможно будет запустить обучение/инференс или NLP-модель — такая машина будет считаться заблокированной.*
