...
Администратор может создать [целевую машину](https://docs.primo-rpa.ru/primo-rpa/primo-rpa-ai-server/glossary#celevaya-mashina), изменить, выключить (или, иначе, заблокировать), включить, удалить из системы. Чтобы выключить или включить машину, администратор должен обладать правом «Агенты — Управление». У встроенных административных ролей это право есть по умолчанию.
...
* **Доступна** — указывает на доступность физической/виртуальной целевой машины. Возможные значения:
    * `Да` — машина доступна, а под статусом отображается время последнего опроса машины. Период опроса настраивается в конфигурационном файле Api в параметре **Agents > Period**.
    * `Нет` — машина недоступна. В этом случае рекомендуется проверить настройки конфигурационного файл агента данной машины. На недоступной машине пользователь не сможет запустить процессы инференса/обучения или NLP-модель, даже при условии, что машина включена. 
...
Чтобы пользователь в рамках проекта смог запускать процессы инференса/обучения модели (*Умный OCR*) или запускать NLP-модель, администратору нужно включить целевую машину. По умолчанию все созданные машины включены. 
...
:large_orange_diamond:***Важно**. Спустя минуту после выключения машины освободится лицензия, которую занимает ее агент. Если машина выключена, то на ней невозможно будет запустить обучение/инференс или NLP-модель.*
...
Для выключенной машины в столбце **Доступна** будет отображаться статус `Выключена`, а также дата и время отключения. 

![](<../../../.gitbook/assets1/primo-ai/admin/machine-off.png>)
