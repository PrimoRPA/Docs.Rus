...
В Primo RPA AI Server существуют учетные записи, которые создаются автоматически при первом запуске приложения — это *встроенные пользователи* системы. Их список вы можете посмотреть см. в разделе [Встроенные пользователи и роли](https://docs.primo-rpa.ru/primo-rpa/primo-ai-server/admin/system-users).
...
3. При необходимости настройте дополнительные параметры:
   * **Роль** — выберите из выпадающего списка [роль пользователя](https://docs.primo-rpa.ru/primo-rpa/primo-ai-server/admin/roles). Пользователь может иметь несколько ролей. Если нужной роли нет или вы не хотите ее указывать сейчас, то роль можно назначить позднее при редактировании пользователя.
