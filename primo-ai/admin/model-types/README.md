...
Если вы хотите изменить конфигурацию по умолчанию, то разверните блок **Конфигурация** в форме создания типа модели:
   ...
   * **Plot** — определяет, выключена ли отладка. По умолчанию параметр выключен - отладка выключена. Для отправки файлов техподдержке (где искать файлы? где-то в корне проекта - при обращении к техподдержке)... Если плохо инференс работает, но явных ошибок и исключений нет. 
   * **Multiprocessing** — количество процессов, используемое для обработки. Значение по умолчанию не задано - идет обращение к системе, автоматически вычисляется кол-во процессоров (загружает половину ядер - 1 для диспетчеризации). Лучше оставлять по умолчанию.
   ...
   * **AllBboxesInResponse** — есть в релизном UI или нет? для эни текста делалось.
