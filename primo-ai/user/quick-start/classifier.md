...
1. Дождитесь, когда статус изменится на «Выполняется» (по-прежнему обновляя страницу или оно само сейчас обновляется?).

![](<../../../.gitbook/assets1/primo-ai/class-training-process-is-running.png>) 

В результате запустится процесс обучения модели на целевой машине. Это длительный процесс, который, как правило, занимает несколько часов. Чтобы узнать приблизительное время обучения вашей модели, разверните справочную информацию о процессе, нажав на **v**. В колонке «~До окончания» вы увидите примерное время, оставшееся до завершения процесса. 

![](<../../../.gitbook/assets1/primo-ai/class-training-process-time-until.png>) 
...
1. Перейдите на страницу «Модели» — в таблице отобразится имя вашей обученной модели. 

   ![](<../../../.gitbook/assets1/primo-ai/>)

Обученная модель представляет собой архив с .pth-файлом модели, а также её метаданными – config.yaml.
