...
## Стадии работы 
1. Обучение модели-классификатора.
2. Настройка и запуск инференса.
...
***Примечание**. Поля могут называться иначе, но должны передавать взаимосвязь с типом документа*.
...
1. Выделите инструментом **Add Bounding Box (b)** значимую часть документа — область с данными. 
...
1. Перейдите к следующему изображению и выберите тип поля в соответствии с типом документа на изображении. Например, если следующее изображение относится к паспорту, то выберите поле passport.
...
1. По аналогии с предыдущими документами выполните разметку для изображения паспорта. Сохраните разметку в датасете «Обучение».
1. Завершите разметку оставшихся изображений проекта. Добавляя изображение в датасет, не забывайте каждый раз сохранять разметку.
