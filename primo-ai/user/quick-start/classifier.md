...
1. Дождитесь смены статуса на «Выполняется» (по-прежнему обновляя страницу или оно само сейчас обновляется?).

   ![](<../../../.gitbook/assets1/primo-ai/>) 

В результате запустится процесс обучения модели на целевой машине. Это длительный процесс, который, как правило, занимает несколько часов.
