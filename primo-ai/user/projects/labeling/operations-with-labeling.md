...
* **Select (v)** — позволяет выбрать на изображении размеченное поле, чтобы применить к нему какое-либо действие. Например, удалить (клавишей `Del`), скорректировать выделенную область, перетащить в другое место изображения (потянув за точку в центре).
