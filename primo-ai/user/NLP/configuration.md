...
1. **Размер контекстного окна** — объем текста, который модель рассматривает перед генерацией текста. Это предельное количество токенов (текстовых единиц), которые могут быть указаны во входном тексте для обработки. По умолчанию `4096`.
