...
Страница настройки открывается автоматически после создания проекта. Также на нее можно перейти, если выбрать на главной странице карточку NLP-проекта.

Цель выполнения всех настроек — получить модель, готовую обрабатывать текст на естественном языке в соответствии с задачей, которую поставил перед ней пользователь/робот. Потенциальные задачи ограничиваются выбранными навыками модели. 

![](<../../../.gitbook/assets1/windows_items/>) (Скрин страницы с цифрами рабочих областей и поясняшками.)

Настройка состоит из трех основых шагов: 
1. Настройка конфигурации NLP-модели.
1. Запуск модели на целевых машинах.
1. Выбор и настройка навыков, которыми должна обладать модель. 

После того, как модель будет настроена и готова принимать запросы, вы можете воспользоваться вкладкой **Тестирование** (цифра на рисунке). Она позволяет испытать отправку и обработку запросов моделью — 

тестового стенда для отправки запросов и  отправлять запросы и получать ответ от модели, не выходя из проекта — 
...
Все модели надо сконфигурировать до их запуска на целевой машине. 


Используйте эту задачу для настройки конфигурации источников данных процесса, файла журнала и сценария, чтобы дать агенту доступ к отслеживаемой программе.
Все агенты надо конфигурировать до их запуска. У каждого агента должна быть базовая информация конфигурации, например, способ соединения с сервером Tivoli Enterprise Monitoring Server. Во многих случаях у агента должна быть также дополнительная информация конфигурации, чтобы иметь доступ к данным конкретной системы, в которой он работает. Например, если вам нужно знать, где установлен программный продукт, то добавьте свойства конфигурации для запроса этой информации. Другой пример запрашиваемой информации - ID пользователя и пароль для доступа к интерфейсу,

Пользовательская конфигурация определяется разработчиком агента. Она не обязательна для всех агентов, но может быть использована в следующих областях сбора данных:
Соответствие аргумента в мониторе процессов
Соответствие командной строки в мониторе процессов
Образование пути или имени файла журнала
Определение переменной среды для сценария
...
1. **Адаптер** — не поддерживается для NLP-задач. 
...
**Добавление целевой машины**:

Выберите одну или несколько целевых машин для запуска NLP-модели. Несколько - чтобы горизонтально масштабировать процесс, каждый агент должен быть лицензирован.

Целевая машина должна соответствовать выбранному устройству в вашей конфигурации.
...
**Остановка NLP-модели на машине**:
