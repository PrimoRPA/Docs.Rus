...
Целевая машина — логическая сущность в Server AI, которая фактически представляет собой физическую или виртуальную машину (или группу машин) с агентом. Агент управляет моделью: запускает, остановливает, настраивает, отправляет запросы. 
...
1. Чтобы запустить модель, переведите переключатель вправо.
   ...
   Начнется автоматический процесс запуска.
...
Чтобы удалить целевую машину из проекта, нажмите иконку меню рядом с машиной и выберите **Удалить**.
