...
1. Конфигурация LLM-модели.
1. Запуск модели на целевых машинах.
1. Выбор и настройка навыков модели.  

После того, как модель будет настроена и запущена, вы можете воспользоваться вкладкой **Тестирование** (цифра 6 на рисунке). Там вы сможете отправить тестовые запросы, чтобы увидеть ответ модели и изменить ее поведение при необходимости.

В проекте может быть несколько конфигураций — они добавляются по кнопке **Добавить новую конфигурацию** (цифра 4 на рисунке). //Для чего несколько конфигураций?

Каждую конфигурацию можно переименовать или удалить — соответствующие команды доступны при нажатии иконки меню (цифра 5 на рисунке). 


## Конфигурация 
Каждую модель следует сконфигурировать до запуска на целевой машине. 


**Основные параметры конфигурации:**

1. **Выбор базовой модели** — выберите из списка тип базовой модели. Базовая модель — заранее предобученная модель нейронной сети, которая обучена на обширном корпусе текстовых данных, может понимать и генерировать текст. Базовые модели входят в поставку AI Server, а также могут быть созданы администратором сервиса.
...
1. **Ключ маршрутизации по умолчанию** — используется для маршрутизации запросов к модели без единого навыка. Фактически такая модель будет способна только создавать текст.

**Расширенные параметры конфигурации:**

1. **LLM-движок** — выберите тип движка, который ассоциирован с базовой моделью. Доступные значения:
    * `Vllm` — сервер для базовой модели на Vllm.
    * `Llama` — сервер для базовой модели на Llama.
1. **Размер контекстного окна** — укажите объем текста, который модель рассматривает перед генерацией текста (и только?). По умолчанию `4096`.
1. **Адаптер** — пропустите этот параметр, он не поддерживается для NLP-задач.
1. **Выбор устройства** — выберите тип устройства, который используется на целевой машине с сервером LLM. Доступные значения:
...
LLM-модели работают на целевых машинах — чтобы запустить модель, потребуется сначала выбрать подходящую целевую машину, после чего произвести запуск модели. Чтобы горизонтально масштабировать процесс, вы можете запустить модель сразу на нескольких машинах. 
...
1. Выберите из выпадающего списка название машины.
1. Чтобы запустить модель на машине, переведите переключатель вправо. Начнется автоматический процесс запуска, который состоит из последовательных стадий: запуска Logic-сервера, скачивания и распаковки модели и т.д.
1. Дождитесь статуса `Готова принимать запросы` — модель успешно запустилась и готова к работе.

Теперь вы можете перейти к добавлению навыков NLP-модели.
