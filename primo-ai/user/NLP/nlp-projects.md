...
Цель выполнения настроек — получить модель, готовую принимать NLP-запросы. Задачи, которые может решать модель, ограничиваются навыками, которые вы определите в ходе настройки. 
...
Одной модели можно назначить несколько навыков. При указании навыка потребуется также выбрать ключ маршрутизации — уникальный ключ, с помощью которого сервер будет выбирать модель и навык для обработки поступившего запроса. Нельзя указать одинаковый ключ маршрутизации для разных навыков. 

После указания ключа маршрутизации автоматически загружается файл контекста — встроенный json-файл, который определяет поведение модели с определенным навыком относительно будущих запросов. Файл контекста берется из профиля ключа маршрутизации — например, для всех встроенных ключей этот файл предзадан. Также файл может отсутствовать, если ключ был создан администратором, который не стал при создании ключа загружать файл. 

Структура файла контекста (.json):
* Секция `"input"` — пример входного текста, который может быть указан в запросе для определенного навыка.
* Секция `"keys"` — ключевые слова, которые помогают модели сосредоточиться на наиболее релевантной информации в тексте запроса.
* Секция `"output"` — пример ответа, который подсказывает модели корректный вариант обработки текста, а также стиль изложения (более или менее формальный).

Таким образом, файл контекста помогает улучшить качество ответов модели, генерировать более релевантные и точные ответы. Вы можете заменить файл контекста, который загрузился из профиля ключа, собственным файлом.
...
1. Укажите ключ маршрутизации запросов. Для каждого навыка должен быть задан уникальный ключ, с помощью которого сервер будет выбирать модель и навык для обработки поступившего запроса.
1. После выбора ключа автоматически отобразится **файл с контекстом** — это встроенный json-файл, который определяет поведение модели с определенным навыком относительно будущих запросов. Вы можете:
