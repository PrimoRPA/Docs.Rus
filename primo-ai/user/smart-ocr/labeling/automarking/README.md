# Инструкция по авторазметке данных
...
* `Бездействует` — пользователь создал процесс, но пока не применил к нему действий.
* `Готов к запуску` — пользователь запустил процесс.
* `Запуск обучения` — запускается процесс обучения на стороне целевой машины.
* `Обучение` — выполняется обучение.
* `Запуск разметки` — процесс готов к переходу в следующую фазу.
* `Разметка` — выполняется разметка: модель предсказывает BBox'ы на неразмеченных изображениях.
* `Завершен` — процесс успешно завершен.
* `Остановка` — пользователь остановил процесс.
