...
* `Бездействует` — процесс не запущен.
* `Готов к запуску - архивация неразмеченных изображений`
* `Запуск процесса обучения на стороне целевой машины`
* `Выполняется обучение`
* `Выполняется предсказание BBox'ов`
* `Выполняется предсказание BBox'ов`
* `Завершен успешно`
* `Ожидает остановки`
* `Ошибка`
* `Таймаут ожидания состояния от процесса истек`
