...
В конфигурационном файле `appsettings.ProdLinux.json` укажите адрес Primo.AI.Api и его компонентов, уникальный идентификатор агента, а также TenantId (если эта машина не в тенанте по умолчанию) и пользователя тенанта:
```
 	"Api": {
...
Уникальный идентификатор агента затем следует указать в разделе **Настройки ➝ Целевые машины** Руководства администратора **Primo RPA AI Server**.
