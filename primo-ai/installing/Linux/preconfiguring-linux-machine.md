...
Документ предназначен для системных администраторов, имеющих следующие навыки и знания:
...
Порт 44392, указанный на рисунке выше, далее используется при настройке конфигурационных файлов агента и открытия портов на файерволе (в том числе аппаратном в сети организации). Для настройки целевой машины нужна чистая  машина с ОС Linux (обязательно с последними обновлениями). И на неё должна быть скопирована папка с комплектом поставки (см. ниже «Комплект поставки»). Это может быть любая папка, для определенности, пусть будет папка `/srv/samba/shared/install`.
...
Для настройки целевой машины потребуются файлы из комплекта поставки Primo RPA AI Server, которые приведены в Таблице 1. Остальное ПО должно быть предустановлено в Linux.

#### Таблица 1 – Файлы из комплекта поставки Primo RPA AI Server для настройки Целевой машины
...
Для целевой машины следует использовать рабочую станцию под управлением Linux, к которой предъявляются требования из Таблицы 2.
...
1. На экране «Настройка учётных записей и паролей» создать пользователя-администратора (далее - primo-admin).
