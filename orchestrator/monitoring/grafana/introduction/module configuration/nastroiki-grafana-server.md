# Настройки Grafana Server

Конфигурация Prometheus Server описывается в файле **%mon\_home%\grafana\grafana\conf\defaults.ini**

Файл **defaults.ini** Grafana является конфигурационным файлом по умолчанию для Grafana, который содержит настройки и параметры, используемые для настройки работу данного инструмента мониторинга и анализа данных.

Основные разделы файла **defaults.ini** включают:

* \[**server**]: этот раздел содержит настройки для сервера Grafana - адрес, порт и протокол, используемые для подключения к серверу.
* \[**database**]: в этом разделе настраиваются параметры базы данных, используемой для хранения данных Grafana.
* \[**security**]: этот раздел содержит настройки безопасности, настройки аутентификации и авторизации.
* \[**analytics**]: настройка отправка телеметрии на серверы grafana.labs. Отправка телеметрии по-умолчанию отключена.
* \[**snapshots**]: этот раздел содержит настройки для создания снимков, которые позволяют сохранять текущее состояние дашборда.
* \[**dashboards.json**]: здесь определяются настройки для дашбордов, включая путь к файлу конфигурации и параметры отображения.
* \[**smtp**]: в этом разделе определяются настройки для отправки электронной почты - сервер SMTP и учетные данные.
* \[**logging**]: здесь определяются настройки для логирования - уровень логирования и формат вывода.
* \[**metrics**]: этот раздел содержит настройки для метрик - адрес и порт, используемые для отправки метрик.
