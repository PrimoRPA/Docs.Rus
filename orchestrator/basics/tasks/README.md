...
* **Использовать готовое** - обязательный параметр. В нем выбираем готовое расписание, заданное Cron-строкой или конструктором. Чтобы расписание было доступно для выбора, его предварительно нужно добавить в [справочник расписаний](https://docs.primo-rpa.ru/primo-rpa/orchestrator/basics/tasks/schedules).
* **Завершение робота** - опциональный параметр. Позволяет завершить по расписанию уже запущенное задание. Всегда должен идти в паре с обычным триггером на запуск задания по расписанию, иначе не имеет смысла (нечего завершать).

  :small_blue_diamond: ***Когда может пригодиться завершение?***

  Триггер с включенным **Завершением робота** можно рассматривать как предохранитель. Допустим, робот выполняет задание ~10 минут, но иногда может работать и дольше. В этом случае можно запускать его каждые 15 минут (первый триггер) и через 10 минут завершать по второму триггеру. Таким образом, в одном задании мы будем иметь 2 триггера по расписанию: 
  * в первом выбрано расписание «каждые 15 минут», которое запускает робота;
  * во втором выбрано расписание «каждые 15 минут», но сдвинутое на 10 минут относительно первого - оно будет завершать работу робота. Во втором триггере установлен чекбокс **Завершение робота**.

**Как создать задание с триггером только на запуск:**

1. Создаем задание с триггером **Запуск по расписанию**.
2. В параметре **Использовать готовое** выбираем расписание, согласно которому будет запускаться задание.
3. Чекбокс **Завершение робота** оставляем отключенным.
4. Сохраняем задание.


**Как создать задание с настроенным запуском и завершением:**
...
2. Второй триггер управляет завершением задания:
   * Выбираем расписание в параметре **Использовать готовое**. По нему задание будет останавливаться в случае, если к тому моменту еще выполняется.
   * Включаем чекбокс **Завершение робота** и выбираем, как следует остановить робота:
     * если мягко - выбираем **Попросить остановиться**. Мягкую остановку рекомендуется применять во избежание ошибок. Чтобы остановка сработала, в RPA-проект обязательно помещаем элемент [**Должен остановиться**](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_orch/els_process/el_shouldstop) (Should stop).
