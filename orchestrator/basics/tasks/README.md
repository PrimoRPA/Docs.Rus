...
* **Использовать готовое** - обязательный параметр. Выберите из списка готовое расписание, заданное Cron-строкой или конструктором. Чтобы расписание было доступно для выбора, его предварительно добавляют в [справочник расписаний](https://docs.primo-rpa.ru/primo-rpa/orchestrator/basics/tasks/schedules).
* **Завершение робота** - опциональный параметр. Позволяет завершить уже запущенное задание при наступлении времени из расписания. Всегда должен идти в паре с обычным триггером, который запускает задание по расписанию, иначе не имеет смысла (нечего завершать).

  :small_orange_diamond: ***Если чекбокс «Завершение робота» отключен, то выполнится только запуск задания!***

**Как создать задание с завершением:**

1. Создаем задание и устанавливаем триггер **Запуск по расписанию**. Выбираем расписание, а чекбокс **Завершение робота** оставляем отключенным - этот триггер сработает только на запуск задания.
2. Добавляем в это задание второй триггер **Запуск по расписанию**. Включаем для него чекбокс **Завершение робота** и выбираем расписание, по которому должно сработать завершение задания (если оно еще выполняется). Выбираем, как следует остановить робота:
   * мягко - тогда выбираем **Попросить остановиться**. Рекомендуется применять мягкую остановку во избежание потенциальных ошибок. Чтобы мягкая остановка сработала, в RPA-проект обязательно помещаем элемент [**Должен остановиться**](https://docs.primo-rpa.ru/primo-rpa/g_elements/el_basic/els_orch/els_process/el_shouldstop) (Should stop).
   * принудительно - выбираем значение **Принудительное**. В этом случае в проекте не требуется наличие элемента **Должен остановиться**.
4. Сохраняем задание.

**Когда может пригодиться завершение:**
