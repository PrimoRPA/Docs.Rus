...
* **Завершение робота** - опциональный параметр. Определяет, нужно ли принудительно, по расписанию, завершать запущенное задание. По умолчанию чекбокс выключен - завершение задания не используется. 

:small_blue_diamond: ***Когда может пригодиться завершение?***

Включенный чекбокс **Завершение робота** используется как предохранитель. Допустим, робот выполняет задание ~10 минут, но иногда может и дольше. В этом случае можно запускать его каждые 15 минут - это первый триггер, и через 10 минут завершать по второму триггеру. Таким образом, в одном задании мы будем иметь 2 триггера по расписанию: 
...
***Завершение робота** всегда нужно использовать в паре с обычным триггером на запуск задания по расписанию, иначе он не имеет смысла (нечего завершать).*
...
#### Как создать задание с запуском и завершением:
