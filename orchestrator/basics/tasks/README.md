...
Триггеры позволяют запускать RPA-проекты по событию, образуя реактивные цепочки исполнения и/или периодическое исполнение проектов. Имеются следующие триггеры:

### 1. Запуск по расписанию
Триггер срабатывает по фиксированному расписанию. Имеет параметры:
* **Использовать готовое** - выберите расписание, заданное Cron-строкой или конструктором. Нужное расписание предварительно должно быть добавлено в [справочник расписаний](https://docs.primo-rpa.ru/primo-rpa/orchestrator/basics/tasks/schedules).

Триггер также возможно настроить на закрытие робота по расписанию, если робот уже выполняет задание. Для этого нужно включить параметр триггера:
* **Завершение робота** - при наступлении времени из расписания процесс робота завершится. Рекомендуется выбирать мягкую остановку робота (**Попросить остановиться**) во избежание возможных ошибок.
