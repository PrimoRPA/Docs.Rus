...
| Физическое удаление      | Параметр может отсутствовать - его видимость зависит от [настройки](https://docs.primo-rpa.ru/primo-rpa/orchestrator/deployment/fine-tuning/physic-removing-items) конфигурационного файла. Переключатель «Физическое удаление» определяет, как элемент будет удален из очереди - логически или физически. Логическое удаление задано по умолчанию. В этом случае удаленные элементы остаются в БД, а также видны в UI Оркестратора - они подсвечиваются красным. При физическом удалении элементы удаляются из БД и недоступны для просмотра в интерфейсе |
