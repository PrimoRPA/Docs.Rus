...
Ниже приведены общие схемы алгоритмов для обработки очереди. Параметр «Кол-во попыток при извлечении элемента по FIFO» используется, чтобы можно было обрабатывать очередь по схеме, где возврат NULL означает пустую очередь (схема «а»). Принятие решения о завершении на схеме не показано.
