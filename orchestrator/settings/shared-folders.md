...
:small_orange_diamond: ***По умолчанию робот обращается к объектам Оркестратора в контексте текущей папки. Все объекты, которые нужны роботу, должны также находиться в текущей папке. Для обращения используется ID папки. Возможность явно указывать папку в элементах Студии появится в будущих релизах***.
