...
1. **Кому\*** *[String]* - адресат сообщения. Обязательный параметр. Пример: `"recipient"`. В случае, если указан только адресат, а все остальные параметры не заполнены, то будет отправлено пустое письмо. 
1. **Кому (в копии)** *[String]* - укажите, кого следует поставить в копию.
1. **Скрытая копия** *[String]* - кого поставить в скрытую копию.      
...
1. **Папка копирования** *[String]* - название папки для сохранения копии письма после отправки. 
