...
1. **Вложения** *[System.Collections.Generic.List\<String>]* - пути к файлам вложений письма. Указать пути можно несколькими способами:
   ...
   * Использовать редактор коллекций (1). В поле редактора укажите путь до файла либо заранее созданную переменную, содержащую этот путь. По нажатию клавиши `Enter` добавляется новая строка, куда можно вписать путь/переменную до другого файла.
