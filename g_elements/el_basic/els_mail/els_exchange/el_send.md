...
1. **Копировать в папку** *[Boolean]* - определяет, нужно ли сохранять копию письма в папке «Отправленные». Если папку нужно изменить, дополнительно используйте свойство **Папка копирования**. По умолчанию чекбокс выключен - копия не сохраняется.
1. **Папка копирования** *[String]* - определяет, в какую папку нужно сохранять копию письма. Работает только при установке чекбокса **Копировать в папку**. 
