...
Элемент для получения почтовых сообщений по протоколу IMAP.

В таблице ниже приведены свойства элемента.\
...
| Программная сортировка      | Boolean                                           | Параметр включает сортировку писем на стороне робота. Позволяет использовать сортировку в тех случаях, когда почтовый сервер ее не поддерживает (например, mail.ru). **Ограничение**: не рекомендуется использовать эту функцию при работе с большим объемом писем, так как будет существенно расходоваться оперативная память
