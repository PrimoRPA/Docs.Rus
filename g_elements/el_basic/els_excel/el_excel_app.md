...
Элемент производит подключение к приложению Microsoft Excel. Параметры подключения и настройки для работы с файлом указываются в свойствах элемента.

Особенности работы с файлами зависят от типа драйвера:

1. **DX** — драйвер работает с файлом без Microsoft Office, вследствие чего имеет более высокую производительность. Хорошо подходит для небольших файлов, поддерживается на всех указанных ОС. Недостатки драйвера:
   * не работает с макросами;
   * может занимать много памяти, если файл большой.
3. **Interop** — драйвер работает с файлом через запуск приложения. Поддерживает макросы, не занимает много оперативной памяти. Недостатки: работает медленнее, чем DX, не поддерживается на Linux.


В сценарии **Приложение Excel** выступает контейнером для других элементов, работающих с файлом Excel.

:small_orange_diamond: ***Важно***. *Чтобы изменения в файле не потерялись, обязательно используйте элемент [Сохранить документ](https://docs.primo-rpa.ru/primo-rpa/g\_elements/osnovnye-elementy/prilozhenie-excel/el\_excel\_save).*
...
| Присоединяться         | Boolean                                                                                                | <p>Только для Interop. Установка чекбокса позволяет присоединиться к файлу, который открыт вручную.</p><p><span data-gb-custom-inline data-tag="emoji" data-code="1f538">🔸</span> <strong>Важно.</strong> Если вы присоединились к такому файлу, то закрывать его тоже потребуется вручную</p>  |
