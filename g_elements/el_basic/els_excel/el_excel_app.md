...
## Загрузка надстроек
Для загрузки надстроек, используемых в файле, существует специальное свойство **Загружать AddIn-ы** (Interop) - при его включении будет совершена попытка загрузить все надстройки сразу. Но бывают случаи, когда использование этого свойства не помогает загрузить нужную надстройку. Например, если для нее нужны права администратора\*, или если есть конфликт с центром безопасности MS Office. 
...
Есть 2 типа надстроек: 
1. Надстройки COM - ставятся для всех пользователей сразу, и для того, чтобы их установить/удалить нужны права администратора. Поэтому не работает свойство **Загружать AddIn-ы**. 
2. Надстройки Еxcel - могут устанавливаться обычными пользователями, но тоже есть нюансы. 
возможно имеет смысл модернизировать саму активность и указывать ту надстройку которую надо загрузить и путь откуда надо ее брать.
