...
1. **Режим** — режим ввода текста:
   * `INVOKE` — производит вставку текста. Значение по умолчанию. Окно приложения может быть свернутым.
   * `SIMULATE` — эмулирует ввод текста с клавиатуры и предназначен для тех приложений, где ввод через INVOKE не поддерживается. Окно приложения должно быть развернутым.
