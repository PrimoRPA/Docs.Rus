...
3. Если в сценарии вы сначала используете контейнер **Присоединиться к БД**, в котором нет запросов, а сам запрос поместили ниже, вне контейнера, то в свойствах запроса возможно указать активное подключение. Для этого сохраните в контейнере **Присоединиться к БД** установленное подключение в переменную (свойство вывода **Соединение с БД**), а затем используйте эту переменную в запросе, в свойстве **Соединение с БД**. 
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ | ----------- |
| **База данных:**        | | | |
| Соединение с БД         | LTools.Database.DatabaseInst  | Переменная, хранящая инстанс соединения с БД. Это свойство заполняется, если вы хотите использовать ранее установленное и активное подключение к БД (см. п.3 рекомендаций). Если вы указали переменную, оставьте пустыми свойства **Строка соединения** и **Тип БД**  |
