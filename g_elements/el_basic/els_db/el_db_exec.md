...
Элемент выполняет запрос к базе данных (БД). В сценарии он может располагаться:
...
3. Если в сценарии вы сначала используете контейнер **Присоединиться к БД**, в котором нет запросов, а сам запрос поместили ниже, вне контейнера, то в свойствах запроса вы можете обратиться к активному подключению. Для этого сохраните в контейнере **Присоединиться к БД** активное соединение в переменную (свойство вывода **Соединение с БД**), а затем используйте эту переменную в свойстве запроса **Соединение с БД**.
