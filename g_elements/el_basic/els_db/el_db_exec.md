...
## Рекомендации
 ...
 1. Если работаете с MS SQL, то вместо использования встроенных элементов из группы **База данных** установите nuget-пакет [Primo.Database.SqlServer](https://www.nuget.org/packages/Primo.Database.SqlServer) - в нем есть поддержка именованных параметров для SQL-запроса (через *@Parameter*).
...
| Аргументы (конструктор) | String             | Аргументы запроса в строковом формате. Строку можно сформировать в окне мастера по кнопке ![](<../../../.gitbook/assets/args-constructor.png>). <br></br> Пример результата: `"{\"Args\":[{\"Position\":0,\"Name\":\"@par1\",\"Script\":\"\\\"aaa\\\"\"}]}"`<br></br>Где **Args** - это массив аргументов. Описание аргумента массива см. в подразделе ниже |
