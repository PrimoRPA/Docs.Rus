...
Элемент очереди имеет следующие статусы:
...
2. **In Progress («В обработке»)** - элемент извлечен из очереди. Извлечь его можно, например, с помощью компонента [**Получить из очереди**](https://docs.primo-rpa.ru/primo-rpa/g\_elements/el\_basic/els\_orch/els\_queues/readfromqueue). Это тоже «псевдостатус».
...
| Статус    |                                       -                                      | Статус элемента. Выберите нужное значение в выпадающем списке: Success/Erro/Business Error                                  | `Success`          |
