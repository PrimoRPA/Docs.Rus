...
**«Добавить в очередь» (Add to queue)** - добавляет новый элемент [в очередь Оркестратора](https://docs.primo-rpa.ru/primo-rpa/orchestrator/basics/data-queues). Добавленный элемент будет иметь статус New и ожидать извлечения из очереди Роботом-читателем.
