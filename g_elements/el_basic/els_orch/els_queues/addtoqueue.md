...
| Приоритет     | Int32                       | Приоритет добавляемого элемента очереди. Свойство определяет приоритет элементов наряду со свойством **Недоступен до**. Значение не должно быть отрицательным или дробным. Положительное значение ограничивается лишь типом данных (интовым) |
...
1. **Тэги**. Пример: `List{"tag"}`.
1. **Недоступен до**. Пример: `new DateTime(2022, 9, 19)`.
