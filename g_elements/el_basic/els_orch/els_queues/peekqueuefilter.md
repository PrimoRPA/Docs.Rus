...
| Занимать   | Boolean | Определяет, нужно ли занимать отфильтрованные записи. По умолчанию флаг **НЕ установлен**. В ответе вернутся только те значения, которые можно занять = изъять из очереди, чтобы далее с ними работал только Ваш Робот| 
