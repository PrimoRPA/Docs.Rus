...
2. **Аргументы** ![](<../../../.gitbook/assets/args-link-process2.png>) - нажатие вызывает окно с аргументами подпроцесса. С версии 23.7 допустимо использовать горячие клавиши:
   * для создания переменной - `Ctrl` + `K`. Переменная создается для выбранного аргумента подпроцесса, поэтому ее тип данных будет соответствовать типу аргумента. Созданная переменная отобразится в столбце **Назначение** аргумента, а также на панели переменных основного процесса.
   * для создания аргумента - `Ctrl` + `Alt` + `K`. Аргумент создается для выбранного аргумента подпроцесса. 
