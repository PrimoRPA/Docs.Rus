...
- **Шаблон поиска**. String. Шаблон поиска элемента управления. Если шаблон заполнен через Мастер, оставьте это свойство пустым.
...
Скачайте на [Learning](https://github.com/PrimoRPA/Learning) обучающий проект **StudioActivities**, чтобы просмотреть готовый RPA-процесс по работе с элементом **Прочитать таблицу**. Пример находится в **StudioActivities > Ru > Браузер > Прочитать таблицу.ltw**. Запустите его в Студии.
