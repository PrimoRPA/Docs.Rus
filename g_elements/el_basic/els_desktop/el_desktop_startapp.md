...
Элемент запускает экземпляр приложения рабочего стола в ОС Windows. Поддерживается передача аргументов запуска.
...
1. **Аргументы** *[String]* — аргументы запуска процесса. Возможно указать несколько аргументов через пробел: `"arg1 arg2"`.
