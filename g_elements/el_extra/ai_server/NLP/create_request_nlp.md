...
1. **Min p\*** *[Double]* — минимальный порог вероятности для выбора токенов в ответе, где `0.0` — любой токен, `1.0` — самый вероятный. Под токеном подразумевается единица текста: слово, символ и т.д. Чем выше порог, тем наиболее вероятные токены модель возьмет в ответ. Это позволяет исключить редкие или неуместные слова в ответах. По умолчанию `0.1`.
